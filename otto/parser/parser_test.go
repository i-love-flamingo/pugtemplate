...
func TestParseFile(t *testing.T) {
	...
	assert.EqualValues(t, err.Error(), "(anonymous): Line 1:1 Invalid regular expression: re2: Invalid (?!) <lookahead>")

	_, err = ParseFile(nil, "", `/(?!def)abc/; return`, IgnoreRegExpErrors)
	assert.EqualValues(t, err.Error(), "(anonymous): Line 1:15 Illegal return statement")

	_, err = ParseFile(nil, "/make-sure-file-path-is-returned-not-anonymous", `a..`, 0)
	assert.EqualValues(t, err.Error(), "/make-sure-file-path-is-returned-not-anonymous: Line 1:3 Unexpected token .")
}

func TestParseFunction(t *testing.T) {
	test := func(prm, bdy string, expect interface{}) *ast.FunctionLiteral {
		function, err := ParseFunction(prm, bdy)
		assert.Equal(t, fmt.Sprint(firstErr(err)), fmt.Sprint(expect))
		return function
	}
...
func TestParserErr(t *testing.T) {
    ...
    `, "(anonymous): Line 3:9 Illegal break statement")
	{
		...
		assert.EqualValues(t, parser.position(stmt.From).Column, 9)
		assert.EqualValues(t, parser.position(stmt.To).Column, 16)
		assert.EqualValues(t, parser.slice(stmt.From, stmt.To), "break; ")
	}
...
func TestParser(t *testing.T) {
	test := func(source string, chk interface{}) *ast.Program {
		...
		assert.Equal(t, fmt.Sprint(firstErr(err)), fmt.Sprint(chk))
		return program
	}
	...
	program = test(";", nil)
	assert.EqualValues(t, len(program.Body), 1)
	assert.EqualValues(t, program.Body[0].(*ast.EmptyStatement).Semicolon, file.Idx(1))

	program = test(";;", nil)
	assert.EqualValues(t, len(program.Body), 2)
	assert.EqualValues(t, program.Body[0].(*ast.EmptyStatement).Semicolon, file.Idx(1))
	assert.EqualValues(t, program.Body[1].(*ast.EmptyStatement).Semicolon, file.Idx(2))

	program = test("1.2", nil)
	assert.EqualValues(t, len(program.Body), 1)
	assert.EqualValues(t, program.Body[0].(*ast.ExpressionStatement).Expression.(*ast.NumberLiteral).Literal, "1.2")
	...
	program = test("\n", nil)
	assert.EqualValues(t, len(program.Body), 0)
...
func Test_parseStringLiteral(t *testing.T) {
	test := func(have, want string) {
		...
		assert.EqualValues(t, err, nil)
		assert.EqualValues(t, have, want)
	}
	...
	test = func(have, want string) {
		have, err := parseStringLiteral(have)
		assert.EqualValues(t, err.Error(), want)
		assert.EqualValues(t, have, "")
	}
...
func Test_parseNumberLiteral(t *testing.T) {
	test := func(input string, expect interface{}) {
		...
		assert.EqualValues(t, err, nil)
		assert.EqualValues(t, result, expect)
	}
...
func TestPosition(t *testing.T) {
	...
	assert.EqualValues(t, parser.slice(0, 1), "")
	assert.EqualValues(t, parser.slice(0, 10), "")

	// Out of range, idx1 (error condition)
	assert.EqualValues(t, parser.slice(1, 128), "")

	assert.EqualValues(t, parser.str[0:0], "")
	assert.EqualValues(t, parser.slice(1, 1), "")

	assert.EqualValues(t, parser.str[0:1], "/")
	assert.EqualValues(t, parser.slice(1, 2), "/")

	assert.EqualValues(t, parser.str[0:14], "// Lorem ipsum")
	assert.EqualValues(t, parser.slice(1, 15), "// Lorem ipsum")
	...
	assert.EqualValues(t, node.Idx0(), file.Idx(2))
	assert.EqualValues(t, node.Idx1(), file.Idx(25))
	assert.EqualValues(t, parser.slice(node.Idx0(), node.Idx1()), "function(){ return 0; }")
	assert.EqualValues(t, parser.slice(node.Idx0(), node.Idx1()+1), "function(){ return 0; })")
	assert.EqualValues(t, parser.slice(node.Idx0(), node.Idx1()+2), "")
	assert.EqualValues(t, node.(*ast.FunctionLiteral).Source, "function(){ return 0; }")
	...
	assert.EqualValues(t, err, nil)
	node = program.Body[0].(*ast.ExpressionStatement).Expression.(*ast.FunctionLiteral)
	assert.EqualValues(t, node.(*ast.FunctionLiteral).Source, "function(){ return abc; }")
	...
	assert.EqualValues(t, node.Idx0(), file.Idx(1))
	assert.EqualValues(t, node.Idx1(), file.Idx(5))
}
